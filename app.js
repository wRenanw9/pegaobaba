// --- FUNÇÕES DE SEGURANÇA E PERFORMANCE SÊNIOR ---
...
function safeParse(data) {
    if (typeof data === 'string') { try { return JSON.parse(data); } catch(e) { return data; } } return data;
}

function customConfirm(title, text, primaryText, secondaryText, primaryColor = 'var(--primary)') {
    return new Promise((resolve) => {
        let elTitle = document.getElementById('modal-confirm-title'); let elText = document.getElementById('modal-confirm-text'); let modal = document.getElementById('modal-confirm-custom');
        if (!modal || !elTitle || !elText) { resolve(confirm(text.replace(/<[^>]*>?/gm, ''))); return; }
        elTitle.innerText = title; elTitle.style.color = primaryColor; elText.innerHTML = text; 
        let btnP = document.getElementById('modal-confirm-btn-primary'); btnP.innerText = primaryText; btnP.style.background = primaryColor; btnP.onclick = () => { modal.style.display = 'none'; resolve(true); };
        let btnS = document.getElementById('modal-confirm-btn-secondary'); btnS.innerText = secondaryText; btnS.style.color = 'var(--dark)'; btnS.style.border = '1px solid var(--border)'; btnS.style.display = 'block'; btnS.onclick = () => { modal.style.display = 'none'; resolve(false); };
...
function customAlert(title, text, btnText="Entendi", primaryColor='var(--primary)') {
    return new Promise((resolve) => {
        let elTitle = document.getElementById('modal-confirm-title'); let elText = document.getElementById('modal-confirm-text'); let modal = document.getElementById('modal-confirm-custom');
        if (!modal || !elTitle || !elText) { alert(text.replace(/<[^>]*>?/gm, '')); resolve(true); return; }
        elTitle.innerText = title; elTitle.style.color = primaryColor; elText.innerHTML = text;
        let btnP = document.getElementById('modal-confirm-btn-primary'); btnP.innerText = btnText; btnP.style.background = primaryColor; btnP.onclick = () => { modal.style.display = 'none'; resolve(true); };
        let btnS = document.getElementById('modal-confirm-btn-secondary'); if(btnS) btnS.style.display = 'none'; 
        modal.style.display = 'flex';
    });
}

function perguntarVencedorPenaltis(idA, nomeA, idB, nomeB) {
    return new Promise((resolve) => {
        let modal = document.getElementById('modal-penaltis');
        let btnA = document.getElementById('btn-penalti-a');
        let btnB = document.getElementById('btn-penalti-b');
        btnA.innerText = `⚽ ${escapeHTML(nomeA)}`;
        btnB.innerText = `⚽ ${escapeHTML(nomeB)}`;
        btnA.onclick = () => { modal.style.display = 'none'; resolve(idA); };
        btnB.onclick = () => { modal.style.display = 'none'; resolve(idB); };
        modal.style.display = 'flex';
    });
}

let db; let jogadores = []; let jogadorEdicaoId = null; let currentProfile = null; let currentUser = null; let supabaseChannel = null; let publicSyncInterval = null;
...
// Tema (claro/escuro/automático). Roda assim que o script carrega, antes de tudo, pra evitar "piscar" no tema errado.
function aplicarTema(tema) {
    if (tema === 'dark' || tema === 'light') { document.documentElement.setAttribute('data-theme', tema); } 
    else { document.documentElement.removeAttribute('data-theme'); }
}
function alterarTema(tema) { try { localStorage.setItem('baba_tema', tema); } catch(e) {} aplicarTema(tema); }
(function() { let temaSalvo = 'auto'; try { temaSalvo = localStorage.getItem('baba_tema') || 'auto'; } catch(e) {} aplicarTema(temaSalvo); })();

window.onload = async function() {
    try {
        let selTema = document.getElementById('select-tema'); if (selTema) { let temaAtual = 'auto'; try { temaAtual = localStorage.getItem('baba_tema') || 'auto'; } catch(e) {} selTema.value = temaAtual; }
        ...
        setInterval(async () => {
            ...
            try { const { error } = await db.from('profiles').select('id').limit(1); if(!error && dbStatus.innerText === "Off-line") { dbStatus.innerText = "Online"; dbStatus.style.backgroundColor = "var(--supabase)"; } } 
            catch(e) { if(dbStatus.innerText !== "Off-line" && dbStatus.innerText !== "Buscando Baba..." && dbStatus.innerText !== "Sincronizando...") { dbStatus.innerText = "Off-line"; dbStatus.style.backgroundColor = "var(--danger)"; } }
        }, 10000);
...
function iniciarOuvinteRealtime(partidaId) {
    ...
    supabaseChannel = db.channel('partida_' + partidaId).on('postgres_changes', { event: '*', schema: 'public', table: 'partidas', filter: `id=eq.${partidaId}` }, payload => { if (window.suprimirProximoEventoRealtime) { window.suprimirProximoEventoRealtime = false; return; } const novaPartida = payload.new; if (novaPartida) processarDadosRecebidosNuvem(novaPartida); }).subscribe();
    if (window.isModoPublico && !publicSyncInterval) { publicSyncInterval = setInterval(async () => { if (!window.partidaAtualId) return; try { const { data: p } = await db.from('partidas').select('*').eq('id', window.partidaAtualId).single(); if (p) processarDadosRecebidosNuvem(p); } catch(e) {} }, 4000); }
}
...
function processarDadosRecebidosNuvem(novaPartida) {
    ...
    atualizarFilaUI(); renderizarSumula();
    let vEst = document.getElementById('view-estatisticas'); if (vEst && vEst.classList.contains('active')) renderizarPainelDoDia();
    let vFin = document.getElementById('view-financeiro'); if (vFin && vFin.classList.contains('active')) gerarRelatorioMensal();
}
    ...
    if(saved) {
        try {
            ...
            if(window.timesSorteadosObjs.length > 0) {
                if(window.codigoAcessoAtual) { exibirBoxCodigoSorteio(window.codigoAcessoAtual); let btnSum = document.getElementById('btn-ir-placares'); if(btnSum) { btnSum.style.display = 'block'; btnSum.innerText = window.partidaSalva ? "📝 Ver Súmula Anterior" : "📝 Preencher Súmula"; } iniciarOuvinteRealtime(window.partidaAtualId); }
                let resDiv = document.getElementById('resultado');
                if(resDiv) {
                    let coringasEmprestadosIds = []; for(let key in window.coringasAtivos) { window.coringasAtivos[key].forEach(c => coringasEmprestadosIds.push(c.jogador.id)); }
                    let resHtml = ""; 
                    window.timesSorteadosObjs.forEach((t) => {
                        let emoji = emojisTimes[coresTimes.indexOf(t.corBase)] || '⚽'; let corHex = getCorHex(t.corBase); let nomeEscaped = escapeHTML(t.nome);
                        let coringasTime = (window.coringasAtivos && window.coringasAtivos[t.id]) ? window.coringasAtivos[t.id] : [];
                        let forcaTime = t.jogadores.reduce((acc, j) => acc + (Number(j.nivel)||3), 0) + coringasTime.reduce((acc, c) => acc + (Number(c.jogador.nivel)||3), 0);
                        
                        let html = `<div class="team" style="border-top-color: ${corHex};"><div style="display:flex; align-items:center; flex-wrap:wrap; gap:5px; margin-bottom:10px;"><span style="font-size:18px; flex-shrink:0;">${emoji}</span><input type="text" value="${nomeEscaped}" onchange="atualizarNomeTime(${t.id}, this.value)" class="input-nome-time" placeholder="Nome do Time" style="color: ${corHex}; font-weight:bold; width:auto; flex:1 1 60px; min-width:0;" ${window.isModoPublico ? 'disabled' : ''}><span class="hidden-public" style="font-size:10px; background:var(--light); padding:3px 6px; border-radius:10px; border:1px solid var(--border); color:var(--text-muted); font-weight:bold; flex-shrink:0;" title="Soma do Nível Técnico">⭐ Força: ${forcaTime}</span></div><ul>`;
                        ...
                        coringasTime.forEach(c => { let posAbbr = posMap[c.jogador.posicao] || c.jogador.posicao; html += `<li style="color: var(--primary); background: var(--tint-primary); margin-left: -5px; padding-left: 5px; border-radius: 4px;"><strong>🎭 ${escapeHTML(c.jogador.nome)}</strong> <span style="font-size:10px;">(do ${escapeHTML(c.timeOriginalNome)})</span> ${c.jogador.posicao!=='Linha'?`<span class="badge badge-posicao" style="display:inline-block; min-width:32px; text-align:center; font-size:9px;">${posAbbr}</span>`:''}</li>`; });
                        resHtml += html + `</ul></div>`;
                    });
...
async function criarConta() { const email = document.getElementById('auth-email').value; const password = document.getElementById('auth-password').value; const msg = document.getElementById('auth-msg'); if(password.length < 6) return msg.innerText = "A senha deve ter 6+ caracteres."; msg.innerText = "Processando..."; const { error } = await db.auth.signUp({ email, password }); if (error) { msg.style.color = "var(--danger)"; msg.innerText = error.message; } else { msg.style.color = "var(--primary)"; msg.innerHTML = "✅ Conta criada! Aguarde a liberação do acesso."; } }
async function fazerLogin() { const email = document.getElementById('auth-email').value; const password = document.getElementById('auth-password').value; const msg = document.getElementById('auth-msg'); msg.innerText = "Conectando..."; const { data, error } = await db.auth.signInWithPassword({ email, password }); if (error) { msg.style.color = "var(--danger)"; msg.innerText = "Credenciais inválidas."; return; } currentUser = data.user; await checarPerfilEValidade(data.user); }

async function checarPerfilEValidade(user) {
    let { data: profile, error } = await db.from('profiles').select('*').eq('id', user.id).single();
    if (error || !profile) { const { data: newProfile, error: insertError } = await db.from('profiles').insert([{ id: user.id, email: user.email, is_authorized: false, is_admin: false, nome_baba: "", jogadores_por_time: 7, despesas_mensais_json: [] }]).select().single(); if (insertError || !newProfile) { await db.auth.signOut(); alert("Erro ao carregar ou criar perfil."); mostrarLogin(); return; } profile = newProfile; }
    if (!profile.is_authorized) { await db.auth.signOut(); alert("Conta não autorizada."); mostrarLogin(); return; }
    if (!profile.is_admin && profile.subscription_expires_at) { if (profile.subscription_expires_at < new Date().toISOString().substring(0, 10)) { await db.auth.signOut(); alert(`Assinatura venceu em ${profile.subscription_expires_at.split('-').reverse().join('/')}.`); mostrarLogin(); return; } }
...
function mostrarApp() { 
    ...
    if (currentProfile && currentProfile.is_admin) { document.body.classList.add('is-master-admin'); let title = document.getElementById('top-bar-title'); if(title) title.innerText = "Painel Master"; mudarAba('view-admin'); } else {
        document.body.classList.remove('is-master-admin'); let title = document.getElementById('top-bar-title'); if(title) title.innerText = currentProfile.nome_baba || "Pega o Baba"; let nomeBaba = document.getElementById('nome-baba-input'); if(nomeBaba) nomeBaba.value = currentProfile.nome_baba || ""; let jogTime = document.getElementById('jogadores-por-time-input'); if(jogTime) jogTime.value = currentProfile.jogadores_por_time || 7;
        if (currentProfile.escudo_url) { let tEsc = document.getElementById('top-bar-escudo'); if(tEsc) { tEsc.src = currentProfile.escudo_url; tEsc.style.display = 'block'; } let pEsc = document.getElementById('preview-escudo'); if(pEsc) { pEsc.src = currentProfile.escudo_url; pEsc.style.display = 'block'; } } else { let tEsc = document.getElementById('top-bar-escudo'); if(tEsc) tEsc.style.display = 'none'; let pEsc = document.getElementById('preview-escudo'); if(pEsc) pEsc.style.display = 'none'; }
        let orgEmail = document.getElementById('organizer-email-label'); if(orgEmail && currentUser) orgEmail.innerText = currentUser.email; 
        carregarPreferenciasFinanceiras(); carregarElencoDaNuvem(); mudarAba('view-sorteio');
    }
}

function mostrarLogin() { document.body.classList.remove('is-master-admin'); let authC = document.getElementById('auth-container'); if(authC) authC.style.display = 'block'; let appC = document.getElementById('app-container'); if(appC) appC.style.display = 'none'; }
async function salvarNomeBaba() { if (!currentUser) return; let novoNome = document.getElementById('nome-baba-input').value.trim(); const { error } = await db.from('profiles').update({ nome_baba: novoNome }).eq('id', currentUser.id); if (error) alert("Erro ao salvar nome: " + error.message); else { alert("✅ Nome do Baba atualizado com sucesso!"); let title = document.getElementById('top-bar-title'); if(title) title.innerText = escapeHTML(novoNome) || "Pega o Baba"; if(currentProfile) currentProfile.nome_baba = novoNome; } }
async function salvarJogadoresPorTime() { if (!currentUser) return; let qtd = parseInt(document.getElementById('jogadores-por-time-input').value) || 7; if (qtd < 3 || qtd > 15) return alert("Insira um número válido entre 3 e 15."); const { error } = await db.from('profiles').update({ jogadores_por_time: qtd }).eq('id', currentUser.id); if (error) alert("Erro ao salvar tamanho das equipes: " + error.message); else { alert("✅ Tamanho das equipes atualizado!"); if(currentProfile) currentProfile.jogadores_por_time = qtd; } }
async function salvarEscudoBaba() { if (!currentUser) return; const fileInput = document.getElementById('input-escudo-baba'); const file = fileInput.files[0]; if(!file) return alert("Por favor, selecione uma imagem no seu celular/computador."); const btn = document.getElementById('btn-salvar-escudo'); btn.innerText = "Enviando... Aguarde"; btn.disabled = true; if (currentProfile && currentProfile.escudo_url) { try { let urlAntiga = currentProfile.escudo_url; let partes = urlAntiga.split('/escudos/'); if (partes.length > 1) await db.storage.from('escudos').remove([partes[1]]); } catch (err) {} } const fileExt = file.name.split('.').pop(); const fileName = `${currentUser.id}_${Date.now()}.${fileExt}`; const { error } = await db.storage.from('escudos').upload(fileName, file, { upsert: true }); if (error) { alert("Erro ao enviar imagem: " + error.message); btn.innerText = "Fazer Upload da Imagem"; btn.disabled = false; return; } const escudoUrl = db.storage.from('escudos').getPublicUrl(fileName).data.publicUrl; const { error: errP } = await db.from('profiles').update({ escudo_url: escudoUrl }).eq('id', currentUser.id); if (errP) alert("Erro ao vincular escudo: " + errP.message); else { alert("✅ Escudo atualizado!"); if(currentProfile) currentProfile.escudo_url = escudoUrl; let tEsc = document.getElementById('top-bar-escudo'); if(tEsc) { tEsc.src = escudoUrl; tEsc.style.display = 'block'; } let pEsc = document.getElementById('preview-escudo'); if(pEsc) { pEsc.src = escudoUrl; pEsc.style.display = 'block'; } } btn.innerText = "Fazer Upload da Imagem"; btn.disabled = false; fileInput.value = ""; }
async function alterarSenhaOrganizador() { const novaSenha = document.getElementById('nova-senha-organizador').value; if(!novaSenha || novaSenha.length < 6) return alert("A senha deve ter pelo menos 6 caracteres."); const { error } = await db.auth.updateUser({ password: novaSenha }); if(error) alert("Erro ao alterar senha: " + error.message); else { alert("✅ Senha alterada!"); document.getElementById('nova-senha-organizador').value = ''; } }
async function zerarHistoricoAdmin() { if(!currentUser) return; let querZerar = await customConfirm("⚠️ Nova Temporada", "Isso vai apagar <strong>TODAS</strong> as partidas, súmulas, caixa e estatísticas.<br><br>O seu Elenco de jogadores será mantido.<br><br>Deseja continuar?", "Apagar Tudo", "Cancelar", "var(--danger)"); if(!querZerar) return; try { let dbStatus = document.getElementById('status-db'); if(dbStatus) dbStatus.innerText = "Limpando..."; const { data: partidas } = await db.from('partidas').select('id').eq('user_id', currentUser.id); if (partidas && partidas.length > 0) { const idsPartidas = partidas.map(p => p.id); await db.from('presencas').delete().in('partida_id', idsPartidas); await db.from('partidas').delete().in('id', idsPartidas); } let querZerarPagamentos = await customConfirm("💰 Zerar Pagamentos?", "Deseja zerar também a contabilidade mensal (Pendente/Pago) dos Mensalistas?", "Sim, zerar tudo", "Não, manter como está", "var(--supabase)"); if(querZerarPagamentos) await db.from('jogadores').update({ pagamentos_json: {} }).eq('user_id', currentUser.id); localStorage.removeItem('baba_full_state'); localStorage.removeItem('baba_presencas_temp'); localStorage.removeItem('baba_last_reset'); window.despesasMensaisGlobais = []; await db.from('profiles').update({ despesas_mensais_json: [] }).eq('id', currentUser.id); await customAlert("✅ Concluído", "O histórico foi apagado com sucesso. O sistema será reiniciado.", "OK", "var(--primary)"); window.location.reload(); } catch (err) { alert("Erro ao limpar histórico: " + err.message); let dbStatus = document.getElementById('status-db'); if(dbStatus) dbStatus.innerText = "Online"; } }

async function carregarPainelAdmin() { if (!currentProfile || !currentProfile.is_admin) return; const container = document.getElementById('lista-organizadores-admin'); if(!container) return; container.innerHTML = "Carregando usuários..."; const { data: users, error } = await db.from('profiles').select('*').order('email'); if (error) { container.innerHTML = "Erro ao carregar usuários."; return; } if (!users || users.length === 0) { container.innerHTML = "Nenhum usuário cadastrado."; return; } let htmlAdmin = ""; users.forEach(u => { let expDate = u.subscription_expires_at || ''; htmlAdmin += `<div style="background: var(--light); padding: 12px; border-radius: 8px; border: 1px solid var(--border); margin-bottom: 10px; font-size: 13px;"><div style="font-weight: 700; margin-bottom: 5px;">${escapeHTML(u.email)} ${u.is_admin ? '(Master Admin)' : ''}</div><div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;"><label style="font-size: 11px;">Autorizado: <input type="checkbox" id="auth-${u.id}" ${u.is_authorized ? 'checked' : ''} onchange="atualizarAdminUser('${u.id}')"></label><label style="font-size: 11px;">Vencimento: <input type="date" id="date-${u.id}" value="${expDate}" style="width: auto; padding: 6px; margin: 0;" onchange="atualizarAdminUser('${u.id}')"></label><span id="msg-${u.id}" style="font-size: 11px; color: var(--supabase); font-weight: bold;"></span></div></div>`; }); container.innerHTML = htmlAdmin; }
async function atualizarAdminUser(uid) { let isAuth = document.getElementById(`auth-${uid}`).checked; let expDate = document.getElementById(`date-${uid}`).value || null; let msgSpan = document.getElementById(`msg-${uid}`); const { error } = await db.from('profiles').update({ is_authorized: isAuth, subscription_expires_at: expDate }).eq('id', uid); if (error) alert("Erro ao atualizar: " + error.message); else { if(msgSpan) msgSpan.innerText = "Salvo!"; setTimeout(() => { if(msgSpan) msgSpan.innerText = ""; }, 2000); } }
...
function sairModoPublico() { if (supabaseChannel) db.removeChannel(supabaseChannel); if (publicSyncInterval) clearInterval(publicSyncInterval); window.history.pushState({}, '', window.location.pathname); window.location.reload(); }

async function carregarElencoDaNuvem() {
    if (!currentUser) return; let dbStatus = document.getElementById('status-db'); if(dbStatus) { dbStatus.innerText = "Sincronizando..."; dbStatus.style.backgroundColor = "var(--warning)"; }
    try {
        const { data, error } = await db.from('jogadores').select('*').eq('user_id', currentUser.id).order('nome', { ascending: true }); if (error) throw error;
        let estadoLocal = {}; try { estadoLocal = JSON.parse(localStorage.getItem('baba_presencas_temp')) || {}; } catch(e) { localStorage.removeItem('baba_presencas_temp'); }
        ...
        if(dbStatus) { dbStatus.innerText = "Online"; dbStatus.style.backgroundColor = "var(--supabase)"; } atualizarListas(); atualizarFinanceiro(); await checarPartidaAtivaAdmin();
    } catch(err) { if(dbStatus) { dbStatus.innerText = "Off-line"; dbStatus.style.backgroundColor = "var(--danger)"; } }
}
    ...
    try {
        ...
        if (!error && partidas && partidas.length > 0) {
            ...
            if (p.created_at && (Date.now() - new Date(p.created_at).getTime()) <= RODADA_EXPIRA_MS) { 
                ...
                if (window.timesSorteadosObjs.length > 0) { if(window.codigoAcessoAtual) exibirBoxCodigoSorteio(window.codigoAcessoAtual); let btnSum = document.getElementById('btn-ir-placares'); if(btnSum) { btnSum.style.display = 'block'; btnSum.innerText = window.partidaSalva ? "📝 Ver Súmula Anterior" : "📝 Preencher Súmula"; } }
                iniciarOuvinteRealtime(window.partidaAtualId); renderizarSumula(); renderizarPainelDoDia(); atualizarFinanceiro();
            }
        }
    } catch(e) {}
}
...
async function marcarTodosPresentes() { let convidadosPendentes = 0; let countAdicionados = 0; let agora = Date.now(); jogadores.forEach((j) => { if (!j.presente) { if (j.tipo === 'Convidado' && !j.pagou) convidadosPendentes++; else { j.presente = true; j.ordemChegada = agora++; countAdicionados++; } } }); if (countAdicionados > 0) { salvarEstadoLocal(); atualizarListas(); } if (convidadosPendentes > 0) await customAlert("Aviso", `<strong>${convidadosPendentes} convidado(s)</strong> não adicionados pois precisam pagar a diária primeiro.`, "Entendi", "var(--warning)"); else if (countAdicionados === 0) await customAlert("Elenco Completo", "Todos os jogadores do elenco já estão prontos pro jogo!", "OK", "var(--primary)"); }
function desmarcarPresenca(indexArray) { jogadores[indexArray].presente = false; jogadores[indexArray].ordemChegada = 0; jogadores[indexArray].pagou = false; salvarEstadoLocal(); atualizarListas(); atualizarFinanceiro(); }
async function alternarMensalidade(idNuvem, indexArray) {
    ...
    if (j.pagamentos_json[mesKey]) { delete j.pagamentos_json[mesKey]; }
    else { let elMens = document.getElementById('valor-mensalista'); let valorPagoAgora = elMens ? (parseFloat(elMens.value) || 0) : 0; j.pagamentos_json[mesKey] = valorPagoAgora; }
    atualizarListas(); atualizarFinanceiro();
    await db.from('jogadores').update({ pagamentos_json: j.pagamentos_json }).eq('id', idNuvem);
}
function alternarPagamentoDiaria(indexArray) { jogadores[indexArray].pagou = !jogadores[indexArray].pagou; if (jogadores[indexArray].tipo === 'Convidado' && !jogadores[indexArray].pagou && jogadores[indexArray].presente) { jogadores[indexArray].presente = false; jogadores[indexArray].ordemChegada = 0; } salvarEstadoLocal(); atualizarListas(); atualizarFinanceiro(); }
async function zerarPresencas() { let conf = await customConfirm("Iniciar Nova Rodada", "Deseja preparar o aplicativo para um <strong>NOVO BABA</strong>?<br><br>Isso vai retirar todos da quadra e zerar as diárias.<br>(A Súmula atual continuará salva até você sortear).", "Sim, Iniciar Novo Baba", "Cancelar", "var(--primary)"); if(conf) { jogadores.forEach(j => { j.presente = false; j.ordemChegada = 0; j.pagou = false; }); salvarEstadoLocal(); atualizarListas(); atualizarFinanceiro(); let res = document.getElementById('resultado'); if(res) res.innerHTML = ""; } }
function filtrarElencoAdmin() { let inp = document.getElementById('busca-jogador-elenco'); if(!inp) return; let termo = inp.value.toLowerCase(); document.querySelectorAll('#lista-elenco-admin .linha-jogador').forEach(linha => { linha.style.display = linha.getAttribute('data-nome').toLowerCase().includes(termo) ? 'grid' : 'none'; }); }
function filtrarSorteioAusentes() { let inp = document.getElementById('busca-jogador-sorteio'); if(!inp) return; let termo = inp.value.toLowerCase(); document.querySelectorAll('#lista-aguardando .linha-jogador').forEach(linha => { linha.style.display = linha.getAttribute('data-nome').toLowerCase().includes(termo) ? 'grid' : 'none'; }); }
async function exportarElencoPorPosicao() { if (!jogadores || jogadores.length === 0) return await customAlert("Aviso", "Não há jogadores cadastrados no elenco.", "OK", "var(--warning)"); let grupos = { "Goleiro": [], "Zagueiro": [], "Lateral": [], "Meia": [], "Atacante": [], "Linha": [] }; jogadores.forEach(j => { let pos = j.posicao || "Linha"; if (!grupos[pos]) grupos[pos] = []; grupos[pos].push(j.nome); }); let nomeBaba = currentProfile && currentProfile.nome_baba ? currentProfile.nome_baba : "Pega o Baba"; let texto = `📋 *Resumo do Elenco - ${nomeBaba}*\n`; texto += `👥 Total de Jogadores: ${jogadores.length}\n\n`; const icones = { "Goleiro": "🧤", "Zagueiro": "🛡️", "Lateral": "🏃", "Meia": "🧠", "Atacante": "🎯", "Linha": "⚽" }; for (let pos in grupos) { if (grupos[pos].length > 0) { grupos[pos].sort((a, b) => a.localeCompare(b)); texto += `${icones[pos] || "⚽"} *${pos}s (${grupos[pos].length})*:\n`; grupos[pos].forEach(nome => { texto += `• ${nome}\n`; }); texto += `\n`; } } texto += `🔗 Gerado pelo app Pega o Baba`; let textoEncoded = encodeURIComponent(texto); let querCompartilhar = await customConfirm("📊 Elenco Exportado", "O resumo foi gerado com sucesso!<br><br>Escolha como deseja exportar:", "📱 WhatsApp", "📋 Copiar Texto", "var(--supabase)"); if (querCompartilhar) { window.open(`https://api.whatsapp.com/send?text=${textoEncoded}`, '_blank'); } else { try { await navigator.clipboard.writeText(texto); customAlert("Copiado!", "O resumo do elenco foi copiado para a área de transferência.", "OK", "var(--primary)"); } catch(err) { alert("Não foi possível copiar automaticamente para o seu dispositivo."); } } }
...
function atualizarListas() {
    ...
    let htmlElenco = ""; let htmlAguardando = ""; let htmlPresentes = ""; let presentes = []; const mesKey = new Date().toISOString().substring(0, 7);
    let todosOrdenados = [...jogadores].sort((a, b) => (a.nome || "").localeCompare(b.nome || ""));
    todosOrdenados.forEach((j) => {
        let indexArray = jogadores.findIndex(jog => jog.id === j.id); let isPago = j.tipo === 'Mensalista' ? !!(j.pagamentos_json && j.pagamentos_json[mesKey]) : j.pagou;
        let colorStyle = isPago ? 'color: inherit;' : 'color: var(--danger); font-weight: 700;'; let clickAction = j.tipo === 'Mensalista' ? `onclick="alternarMensalidade('${j.id}', ${indexArray})"` : `onclick="alternarPagamentoDiaria(${indexArray})"`;
        let publicDisabled = window.isModoPublico ? "" : clickAction; let cursorStyle = window.isModoPublico ? "" : "cursor: pointer;"; let tagConvidado = j.tipo === 'Convidado' ? '<span class="badge badge-convidado">C</span>' : '';
        let posAbbr = posMap[j.posicao] || j.posicao; let tagPosicao = (j.posicao !== 'Goleiro' && j.posicao !== 'Linha') ? `<span class="badge badge-posicao" style="display: inline-block; min-width: 28px; text-align: center;">${posAbbr}</span>` : ''; let nomeSeguro = escapeHTML(j.nome || "Sem Nome");
        if(listaElenco) { let btnEdicao = `<button class="btn-small btn-acao-outline" onclick="prepararEdicao(${indexArray})">✏️</button>`; let btnExcluir = `<button class="btn-small btn-acao-outline" style="color:var(--danger); border-color:var(--danger);" onclick="removerJogador('${j.id}', ${indexArray})">X</button>`; let btnMensal = j.tipo === 'Mensalista' ? (isPago ? `<button class="btn-small btn-pagou" onclick="alternarMensalidade('${j.id}', ${indexArray})">Pago</button>` : `<button class="btn-small btn-devendo" onclick="alternarMensalidade('${j.id}', ${indexArray})">Pendente</button>`) : `<span>Diária local</span>`; htmlElenco += `<div class="linha-jogador grid-elenco" data-nome="${nomeSeguro}"><div class="col-nome">${tagConvidado} <span style="margin-left: 4px;">${nomeSeguro}</span></div><div class="col-status">${btnMensal}</div><div class="col-acoes">${btnEdicao} ${btnExcluir}</div></div>`; }
        if(j.presente) presentes.push({ ...j, indexArray: indexArray, nomeView: nomeSeguro });
        else if(listaAguardando) { let btnAcoesAguardando = ''; if (j.tipo === 'Convidado') btnAcoesAguardando += isPago ? `<button class="btn-small btn-pagou" onclick="alternarPagamentoDiaria(${indexArray})">Pago</button>` : `<button class="btn-small btn-devendo" onclick="alternarPagamentoDiaria(${indexArray})">Pendente</button>`; htmlAguardando += `<div class="linha-jogador grid-aguardando" data-nome="${nomeSeguro}"><div class="col-nome" style="justify-content: space-between; width: 100%;"><div style="display:flex; align-items:center; gap:5px; overflow:hidden; flex: 1; min-width: 0; ${colorStyle} ${cursorStyle}" ${publicDisabled} title="${isPago ? 'Pago' : 'Pendente - Clique para pagar'}">${tagConvidado} <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; display:block;">${nomeSeguro}</span></div>${tagPosicao ? `<div style="flex-shrink:0; margin-left: 5px;">${tagPosicao}</div>` : ''}</div><div class="col-status">${btnAcoesAguardando}</div><div class="col-acoes"><button class="btn-small btn-presente" onclick="marcarPresenca(${indexArray})">Chegou</button></div></div>`; }
    });
    ...
    presentes.forEach((j, pos) => { let tagConvidado = j.tipo === 'Convidado' ? '<span class="badge badge-convidado">C</span>' : ''; let isPago = j.tipo === 'Mensalista' ? !!(j.pagamentos_json && j.pagamentos_json[mesKey]) : j.pagou; let colorStyle = isPago ? 'color: inherit;' : 'color: var(--danger); font-weight: 700;'; let clickAction = j.tipo === 'Mensalista' ? `onclick="alternarMensalidade('${j.id}', ${j.indexArray})"` : `onclick="alternarPagamentoDiaria(${j.indexArray})"`; let publicDisabled = window.isModoPublico ? "" : clickAction; let cursorStyle = window.isModoPublico ? "" : "cursor: pointer;"; let posAbbr = posMap[j.posicao] || j.posicao; let tagPosicao = (j.posicao !== 'Goleiro' && j.posicao !== 'Linha') ? `<span class="badge badge-posicao" style="display: inline-block; min-width: 28px; text-align: center;">${posAbbr}</span>` : ''; htmlPresentes += `<div class="linha-jogador grid-prontos"><div class="col-nome" style="justify-content: space-between; width: 100%;"><div style="display:flex; align-items:center; gap:5px; overflow:hidden; flex: 1; min-width: 0; ${colorStyle} ${cursorStyle}" ${publicDisabled} title="${isPago ? 'Pago' : 'Pendente - Clique para pagar'}"><span class="badge" style="background: var(--accent-dark); color:white; flex-shrink:0;">${pos + 1}º</span> ${tagConvidado} <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; display:block;">${j.nomeView}</span></div>${tagPosicao ? `<div style="flex-shrink:0; margin-left: 5px;">${tagPosicao}</div>` : ''}</div><div class="col-acoes"><button class="btn-small btn-acao-outline hidden-public" style="color:var(--danger); border-color:var(--danger);" onclick="desmarcarPresenca(${j.indexArray})">Retirar</button></div></div>`; });
    if(listaElenco) listaElenco.innerHTML = htmlElenco; if(listaAguardando) listaAguardando.innerHTML = htmlAguardando; if(listaPresentes) listaPresentes.innerHTML = htmlPresentes;
    let elTotalPres = document.getElementById('total-presentes'); if(elTotalPres) elTotalPres.innerText = presentes.length; let elTotalElenco = document.getElementById('total-elenco'); if(elTotalElenco) elTotalElenco.innerText = jogadores.length;
    let btnZerar = document.getElementById('btn-zerar'); if(btnZerar) btnZerar.style.display = "block"; filtrarElencoAdmin(); filtrarSorteioAusentes();
}
...
async function iniciarSorteioComSuspense() {
    ...
    if (window.timesSorteadosObjs.length > 0 && !window.partidaSalva) {
        let jogadoresSemTime = presentes.filter(p => !window.timesSorteadosObjs.some(t => t.jogadores.some(tj => tj.id === p.id)) && !window.reservasSorteados.some(r => r.id === p.id));
        if (jogadoresSemTime.length === 0) {
            ...
            if(acao) { isAppend = true; presentes = jogadoresSemTime; if(presentes.length === 0) return; } 
            else { let confM = await customConfirm("Atenção", "Tem certeza que deseja APAGAR os times atuais e misturar todo mundo de novo? (Não apaga os jogos de hoje).", "Sim, Misturar", "Cancelar", "var(--danger)"); if(!confM) return; }
        }
    }
...
function atualizarFilaTorneio() {
    ...
    let count = validMatches.length;
    let ids = window.timesSorteadosObjs.map(t => t.id);
    if(ids.length < 4) return false;

    let A = ids[0], B = ids[1], C = ids[2], D = ids[3];
    let faseStr = "";

    if (count === 0) { window.filaEquipes = [A, B, C, D]; faseStr = "Grupos (Jogo 1/6)"; }
    else if (count === 1) { window.filaEquipes = [C, D, A, C]; faseStr = "Grupos (Jogo 2/6)"; }
    else if (count === 2) { window.filaEquipes = [A, C, B, D]; faseStr = "Grupos (Jogo 3/6)"; }
    else if (count === 3) { window.filaEquipes = [B, D, A, D]; faseStr = "Grupos (Jogo 4/6)"; }
    ...
    window.faseTorneioTexto = faseStr;
    return true;
}
...
async function sortearTimes(presentesBrutos, isAppend) {
    try {
        ...
        let eqPosCb = document.getElementById('equilibrar-posicoes'); let equilibrarPosicoes = eqPosCb ? eqPosCb.checked : true;
        let selCompeticao = document.getElementById('modo-competicao'); let modoComp = selCompeticao ? selCompeticao.value : 'rei';
        ...
        if (isAppend && (modo === '12' || modo === '14')) { reservasNovas.push(...jogadoresLivres); } 
        else if (!isAppend) { titulares = jogadoresLivres.slice(0, maxTitulares); reservasNovas.push(...jogadoresLivres.slice(maxTitulares)); } 
        else { titulares = jogadoresLivres; }
        ...
        if (titulares.length > 0) {
            ...
            const getSomaNotas = (time) => time.reduce((acc, j) => acc + (Number(j.nivel) || 3), 0); const getQtdPosicao = (time, pos) => time.filter(j => j.posicao === pos).length; const getQtdCracks = (time) => time.filter(j => (Number(j.nivel) || 3) >= 5).length;
            ...
            for (let i = 0; i < titulares.length; i += tamanhoPartida) {
                ...
                if (equilibrarPosicoes) {
                    ...
                    linhaChunk.sort((a, b) => (Number(b.nivel) || 3) - (Number(a.nivel) || 3));
                    linhaChunk.forEach(jogador => {
                        let elegiveis = timesLocais.filter((t, index) => t.length < capacities[index]);
                        if (elegiveis.length === 0) { timesLocais[timesLocais.length - 1].push(jogador); return; }
                ...
                timesLocais = timesLocais.filter(t => t.length > 0); timesNovos.push(...timesLocais);
            }
        }

        if (!isAppend) {
            if (modoComp === 'torneio' && timesNovos.length !== 4) {
                await customAlert("Aviso", `O Modo Torneio exige exatamente 4 times. Foram formados ${timesNovos.length} times.\n\nO baba seguirá automaticamente no formato Rei da Quadra!`, "Entendi", "var(--warning)");
                modoComp = 'rei';
            }
        ...
        timesNovos.forEach((timeArr, idx) => { 
            ...
            timeArr.sort((a,b) => (Number(b.nivel)||3) - (Number(a.nivel)||3));
            let nomeCapitao = timeArr.length > 0 ? "Time de " + timeArr[0].nome.split(' ')[0] : cor;
            window.timesSorteadosObjs.push({ id: globalIdx, corBase: cor, nome: nomeCapitao, jogadores: timeArr, coringas: [] }); 
            if(isAppend && window.modoCompeticaoAtual !== 'torneio') window.filaEquipes.push(globalIdx); 
        });
        
        if (!isAppend) {
            if(window.modoCompeticaoAtual === 'torneio') { atualizarFilaTorneio(); } 
            else { window.filaEquipes = window.timesSorteadosObjs.map(t => t.id); }
        }
        ...
        if (isAppend) {
            ...
            window.suprimirProximoEventoRealtime = true; await db.from('partidas').update({ times_json: window.timesSorteadosObjs, fila_json: window.filaEquipes, quantidade_jogadores: qJogadores, reservas_json: window.reservasSorteados }).eq('id', window.partidaAtualId);
...
async function criarPartidaInicialNoBanco() {
    try {
        if (currentUser) { window.suprimirProximoEventoRealtime = true; await db.from('partidas').update({ codigo_acesso: null }).eq('user_id', currentUser.id); }
        ...
        window.suprimirProximoEventoRealtime = true;
...
async function gerarRelatorioMensal() {
    ...
    else { let htmlMensalistas = `<div class="grid-mensalistas">`; mensalistas.forEach(j => { let vPago = j.pagamentos_json && j.pagamentos_json[mesKey]; let pago = !!vPago; if(pago) totalMensalidades += (typeof vPago === 'number' ? vPago : valMens); let statusText = pago ? `<span style="color:var(--supabase); font-weight:bold;">✅ Pago</span>` : `<span style="color:var(--danger); font-weight:bold;">❌ Pendente</span>`; htmlMensalistas += `<div class="item-mensalista"><span>${escapeHTML(j.nome)}</span> <span>${statusText}</span></div>`; }); htmlMensalistas += `</div>`; html += `<tr><td colspan="2" style="padding: 0;">${htmlMensalistas}</td></tr>`; }
    ...
    html += `<div style="margin-top: 20px; border: 1px solid var(--border); border-radius: 12px; overflow: hidden; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05); page-break-inside: avoid;"><div style="background: var(--accent-dark); color: white; padding: 12px 15px; text-align: center; font-weight: 700; font-size: 14px; text-transform: uppercase; -webkit-print-color-adjust: exact; print-color-adjust: exact;">💰 Resumo Financeiro (${partes[1]}/${partes[0]})</div><div style="padding: 15px; background: var(--white);"><div style="display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 13px;"><span style="color: var(--text-muted);">Entradas no Mês:</span><span style="font-weight: bold; color: var(--supabase);">+ R$ ${receitaTotal.toFixed(2)}</span></div><div style="display: flex; justify-content: space-between; margin-bottom: 12px; font-size: 13px; border-bottom: 1px dashed var(--border); padding-bottom: 12px;"><span style="color: var(--text-muted);">Saídas no Mês:</span><span style="font-weight: bold; color: var(--danger);">- R$ ${totalCustos.toFixed(2)}</span></div><div style="display: flex; justify-content: space-between; font-size: 14px; font-weight: bold;"><span>Saldo do Mês:</span><span style="color: ${saldoMensal >= 0 ? 'var(--primary)' : 'var(--danger)'};">R$ ${saldoMensal.toFixed(2)}</span></div></div><div style="padding: 15px; background: var(--light); border-top: 1px solid var(--border); -webkit-print-color-adjust: exact; print-color-adjust: exact;"><div style="display: flex; justify-content: space-between; margin-bottom: 10px; font-size: 12px; color: var(--text-muted);"><span>⏪ Saldo Anterior Acumulado:</span><span style="font-weight: bold; color: ${saldoAnterior >= 0 ? 'var(--supabase)' : 'var(--danger)'};">R$ ${saldoAnterior.toFixed(2)}</span></div><div style="display: flex; justify-content: space-between; align-items: center; font-size: 16px;"><span style="font-weight: 800; color: var(--dark);">🏦 CAIXA TOTAL:</span><span style="font-weight: 900; font-size: 18px; color: ${saldoTotalCaixa >= 0 ? 'var(--supabase)' : 'var(--danger)'};">R$ ${saldoTotalCaixa.toFixed(2)}</span></div></div></div>`;
...
function exibirBoxCodigoSorteio(codigo) {
    let linkCompartilhamento = `https://pegaobaba.vercel.app?code=${codigo}`; let textoWpp = encodeURIComponent(`🏆 *Acompanhe nosso Baba ao Vivo!*\n\nVeja a súmula, artilharia e classificação da rodada pelo link:\n\n${linkCompartilhamento}\n\n_(Código de acesso direto: ${codigo})_`); let textoWppEscalacao = gerarTextoWhatsAppEscalacao(); let dataBoxStr = window.dataPartidaAtual || new Date().toLocaleDateString('pt-BR', {day: '2-digit', month: '2-digit', year: 'numeric'}); let boxCod = document.getElementById('box-codigo-gerado-sorteio');
    if (boxCod) { boxCod.innerHTML = `<div style="font-size:13px; color:var(--text-muted); font-weight:700; margin-bottom:5px;">Código do jogo (${escapeHTML(dataBoxStr)}) para os jogadores</div><div style="color:var(--primary); font-size:28px; line-height:1; margin-bottom:15px;">${escapeHTML(codigo)}</div><div style="display: flex; flex-direction: column; gap: 8px;"><a href="https://api.whatsapp.com/send?text=${textoWppEscalacao}" target="_blank" class="btn-whatsapp">👥 Compartilhar Escalação no WhatsApp</a><a href="https://api.whatsapp.com/send?text=${textoWpp}" target="_blank" style="display:inline-block; background: var(--accent-dark); color:white; padding:12px 20px; border-radius:8px; text-decoration:none; font-size:14px; font-weight:bold; width:100%; box-sizing:border-box;">📱 Compartilhar Link Geral</a></div>`; boxCod.style.display = 'block'; }
}

function renderizarTimesNaTela() { if (window.timesSorteadosObjs && window.timesSorteadosObjs.length > 0) { let btnSum = document.getElementById('btn-ir-placares'); if(btnSum) { btnSum.style.display = 'block'; btnSum.innerText = window.partidaSalva ? "📝 Ver Súmula Anterior" : "📝 Preencher Súmula"; } } }

function atualizarNomeTime(id, novoNome) { let time = window.timesSorteadosObjs.find(t => t.id === id); if(time) time.nome = novoNome.trim() || time.corBase; salvarEstadoCompleto(); let vPlac = document.getElementById('view-placares'); if (vPlac && vPlac.classList.contains('active')) atualizarSelectsEquipes(); }
...
function renderizarSumula() {
    ...
    if((!window.timesSorteadosObjs || window.timesSorteadosObjs.length === 0) && !hasValidMatch) { if(container) container.style.display = 'none'; if(aviso) aviso.style.display = 'block'; if(dataLabel) dataLabel.innerText = ""; if(instrucoes) instrucoes.style.display = 'none'; return; }
    if(aviso) aviso.style.display = 'none'; if(container) container.style.display = 'block'; if(instrucoes) instrucoes.style.display = window.partidaSalva ? 'none' : 'block'; if(window.codigoAcessoAtual) exibirBoxCodigoSorteio(window.codigoAcessoAtual);
    if(dataLabel) { let dataExibicao = window.dataPartidaAtual || new Date().toLocaleDateString('pt-BR', {day: '2-digit', month: '2-digit', year: 'numeric'}); dataLabel.innerText = `📅 Data da Rodada: ${escapeHTML(dataExibicao)}`; }
    if (window.timesSorteadosObjs && window.timesSorteadosObjs.length > 0 && window.filaEquipes.length === 0 && !window.partidaSalva) {
        if(window.modoCompeticaoAtual === 'torneio') { atualizarFilaTorneio(); } else { window.filaEquipes = window.timesSorteadosObjs.map(t => t.id); }
    }
...
function renderizarEscalacaoPublicaSumula() {
    ...
    window.timesSorteadosObjs.forEach((t) => {
        let emoji = emojisTimes[coresTimes.indexOf(t.corBase)] || '⚽'; let corHex = getCorHex(t.corBase); let coringasTime = (t.coringas && t.coringas.length > 0) ? t.coringas : ((window.coringasAtivos && window.coringasAtivos[t.id]) ? window.coringasAtivos[t.id] : []); let qtdAtual = t.jogadores.length + coringasTime.length;
        let forcaTime = t.jogadores.reduce((acc, j) => acc + (Number(j.nivel)||3), 0) + coringasTime.reduce((acc, c) => acc + (Number(c.jogador.nivel)||3), 0);
        
        let html = `<div class="team" style="border-top-color: ${corHex}; position:relative;"><div style="display:flex; align-items:center; flex-wrap:wrap; justify-content:space-between; gap:6px; margin-bottom:10px;"><div style="display:flex; align-items:center; gap:5px; min-width:0; flex:1 1 auto;"><span style="font-size:18px; flex-shrink:0;">${emoji}</span><input type="text" value="${escapeHTML(t.nome)}" onchange="atualizarNomeTime(${t.id}, this.value)" class="input-nome-time" placeholder="Nome do Time" style="color: ${corHex}; width:auto; font-weight:bold; flex:1 1 50px; min-width:0;" ${window.isModoPublico ? 'disabled' : ''}><span class="hidden-public" style="font-size:10px; background:var(--light); padding:3px 6px; border-radius:10px; border:1px solid var(--border); color:var(--text-muted); font-weight:bold; flex-shrink:0;" title="Soma do Nível Técnico">⭐ Força: ${forcaTime}</span></div>`;
        if(!window.isModoPublico && !window.partidaSalva) { let btnCoringaHtml = `<button onclick="sortearCoringasFila(${t.id})" class="btn-coringa-fila" style="background:var(--primary); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:11px; cursor:pointer; font-weight:bold; flex-shrink:0; white-space:nowrap;">🎭 Coringa</button>`; if(qtdAtual < tamanhoIdeal) { html += btnCoringaHtml; } }
        html += `</div><ul>`;
        t.jogadores.forEach(j => { if(coringasEmprestadosIds.includes(j.id)) return; let posAbbr = posMap[j.posicao] || j.posicao; html += `<li><strong>${escapeHTML(j.nome)}</strong> ${j.posicao!=='Linha'?`<span class="badge badge-posicao" style="display:inline-block; min-width:32px; text-align:center; font-size:9px;">${posAbbr}</span>`:''}</li>`; }); 
        coringasTime.forEach(c => { let posAbbr = posMap[c.jogador.posicao] || c.jogador.posicao; html += `<li style="color: var(--primary); background: var(--tint-primary); margin-left: -5px; padding-left: 5px; border-radius: 4px;"><strong>🎭 ${escapeHTML(c.jogador.nome)}</strong> <span style="font-size:10px;">(do ${escapeHTML(c.timeOriginalNome)})</span> ${c.jogador.posicao!=='Linha'?`<span class="badge badge-posicao" style="display:inline-block; min-width:32px; text-align:center; font-size:9px;">${posAbbr}</span>`:''}</li>`; });
        htmlEscalacao += html + `</ul></div>`;
    });
    
    if (window.reservasSorteados && window.reservasSorteados.length > 0) { let htmlRes = `<div class="team team-reservas"><h3 style="padding:5px; font-size:15px;">Reservas (Inativos)</h3><ul>`; window.reservasSorteados.forEach(j => htmlRes += `<li><strong>${escapeHTML(j.nome)}</strong> ${j.isDM ? '<span style="color:var(--danger); font-size:11px; font-weight:bold;">[DM]</span>' : ''}</li>`); htmlEscalacao += htmlRes + `</ul></div>`; }
    containerEscalacao.innerHTML = htmlEscalacao;
}

function atualizarCoringasUI() { renderizarEscalacaoPublicaSumula(); }
...
async function checarTimesCompletosParaJogo() {
    ...
    if (qA < tamanhoIdeal || qB < tamanhoIdeal) {
        ...
        if (querSortear) { 
            let btnSanfona = document.getElementById('btn-toggle-escalacao'); let containerEsc = document.getElementById('container-escalacao-toggled');
            if (containerEsc && (containerEsc.style.display === 'none' || containerEsc.style.display === '')) toggleEscalacao();
            if (btnSanfona) btnSanfona.scrollIntoView({ behavior: 'smooth', block: 'start' });
            return false; 
        } 
        return true; 
    } return true;
...
async function abrirModalGol(lado) {
    ...
    let selEq = document.getElementById(lado === 'A' ? 'sumula_equipe_a' : 'sumula_equipe_b'); if(!selEq) return alert("Erro: Caixa de equipe não encontrada na tela.");
    const selTimeId = parseInt(selEq.value); if(isNaN(selTimeId)) return alert("Nenhuma equipe em quadra! Gere o sorteio primeiro.");
    const timeObj = window.timesSorteadosObjs.find(t => t.id === selTimeId); if(!timeObj) return alert("Erro: Equipe não encontrada na memória do jogo.");
    const containerJogadores = document.getElementById('lista-jogadores-gol'); if(!containerJogadores) return alert("Lista de jogadores do modal não encontrada.");
    
    let titMod = document.getElementById('titulo-modal-gol'); if(titMod) titMod.innerText = `Gol do(a) ${escapeHTML(timeObj.nome)}`;
    let htmlBotoes = ""; let jogadoresTime = [...timeObj.jogadores].sort((a,b) => (a.nome||"").localeCompare(b.nome||""));
    jogadoresTime.forEach(j => { htmlBotoes += `<button class="btn-jogador-gol" onclick="registrarGol('${lado}', '${escapeJS(j.nome)}')">⚽ ${escapeHTML(j.nome)}</button>`; });
    ...
    coringasTime.forEach(c => { htmlBotoes += `<button class="btn-jogador-gol" style="background: var(--tint-primary); color:var(--primary); border-color:var(--primary);" onclick="registrarGol('${lado}', '${escapeJS(c.jogador.nome)}')">🎭 ${escapeHTML(c.jogador.nome)} (Coringa)</button>`; });
    htmlBotoes += `<button class="btn-jogador-gol" style="background: var(--tint-danger); color:var(--danger); border-color:var(--danger);" onclick="registrarGol('${lado}', 'Gol Contra')">⚠️ Gol Contra</button>`;
...
async function adicionarJogoNaSumula() {
    ...
    try {
        ...
        window.jogosDaRodada.forEach(jogo => {
            ...
            else if (jogo.tipo !== 'modo') { jogo.gols_a.forEach(nome => { if(nome !== 'Gol Contra') artilheiros[nome] = (artilheiros[nome] || 0) + 1; }); jogo.gols_b.forEach(nome => { if(nome !== 'Gol Contra') artilheiros[nome] = (artilheiros[nome] || 0) + 1; }); }
        });
        ...
        if (window.modoCompeticaoAtual === 'torneio') {
            ...
            let idWinner = null; let idLoser = null; 

            if (ga > gb || vencedorPenaltisId === idA) { window.filaEquipes.unshift(idA); window.filaEquipes.push(idB); idWinner = idA; idLoser = idB; } 
            else if (gb > ga || vencedorPenaltisId === idB) { window.filaEquipes.unshift(idB); window.filaEquipes.push(idA); idWinner = idB; idLoser = idA; } 
            else {
                ...
                if(window.coringasAtivos) {
                    [idA, idB].forEach(id => { if(window.coringasAtivos[id]) delete window.coringasAtivos[id]; });
                    let conflitosN1 = []; let conflitosN2 = [];
                    if(window.coringasAtivos[nextTeam1]) { conflitosN1 = window.coringasAtivos[nextTeam1].filter(c => c.timeOriginalId === nextTeam2); if(conflitosN1.length > 0) window.coringasAtivos[nextTeam1] = window.coringasAtivos[nextTeam1].filter(c => c.timeOriginalId !== nextTeam2); }
                    if(window.coringasAtivos[nextTeam2]) { conflitosN2 = window.coringasAtivos[nextTeam2].filter(c => c.timeOriginalId === nextTeam1); if(conflitosN2.length > 0) window.coringasAtivos[nextTeam2] = window.coringasAtivos[nextTeam2].filter(c => c.timeOriginalId !== nextTeam1); }
            ...
            if (!isEmpate && idLoser !== null && window.coringasAtivos) {
                ...
                if (window.coringasAtivos[idWinner] && window.coringasAtivos[idWinner].length > 0) {
                    let nextAdversarioId = window.filaEquipes[1]; let coringasWin = window.coringasAtivos[idWinner]; let conflitos = coringasWin.filter(c => c.timeOriginalId === nextAdversarioId);
                    ...
                    if(window.coringasAtivos[idWinner] && window.coringasAtivos[idWinner].length > 0) {
                        ...
                        if(querRodizio) delete window.coringasAtivos[idWinner];
                    }
                }
            }
        }
...
function removerJogo(index) { 
    ...
    window.jogosDaRodada.splice(index, 1); 
    if (window.modoCompeticaoAtual === 'torneio') { atualizarFilaTorneio(); atualizarSelectsEquipes(); }
    atualizarListaJogosDaRodada(); salvarEstadoCompleto(); 
}
...
function abrirModalLesao() {
    ...
    if(window.reservasSorteados) { window.reservasSorteados.forEach(j => { if(j.isDM) { temGenteNoDM = true; let option = document.createElement('option'); option.value = j.id; option.innerText = j.nome; selAlta.appendChild(option); } }); }
    selAlta.disabled = !temGenteNoDM;
...
async function salvarLesao() {
    ...
    window.timesSorteadosObjs.forEach(t => { t.coringas = window.coringasAtivos[t.id] || []; });
    if(window.partidaAtualId) { window.suprimirProximoEventoRealtime = true; await db.from('partidas').update({ times_json: window.timesSorteadosObjs, reservas_json: window.reservasSorteados }).eq('id', window.partidaAtualId); }
...
function abrirModalAjuste() {
    const sel = document.getElementById('select-jogador-ajuste'); if(!sel) return alert("Erro de HTML: Caixa de seleção não encontrada.");
    sel.innerHTML = '<option value="">Selecione o Jogador</option>'; let todosOrdem = [...jogadores].sort((a,b) => (a.nome||"").localeCompare(b.nome||""));
    todosOrdem.forEach(j => { sel.innerHTML += `<option value="${escapeHTML(j.nome)}">${escapeHTML(j.nome)}</option>`; });
    let inpG = document.getElementById('input-gols-ajuste'); if(inpG) inpG.value = "1";
    let modAj = document.getElementById('modal-ajuste-manual'); if(modAj) modAj.style.display = 'flex'; else alert("Janela de Ajuste de Gols não encontrada no HTML!");
}
...
async function salvarAjusteManual() {
    ...
    window.jogosDaRodada.forEach(jogo => {
        if(jogo.tipo === 'ajuste') { artilheiros[jogo.jogador] = (artilheiros[jogo.jogador] || 0) + jogo.gols; } 
        else if (jogo.tipo !== 'modo') { jogo.gols_a.forEach(n => { if(n !== 'Gol Contra') artilheiros[n] = (artilheiros[n] || 0) + 1; }); jogo.gols_b.forEach(n => { if(n !== 'Gol Contra') artilheiros[n] = (artilheiros[n] || 0) + 1; }); }
    });

    if(window.partidaAtualId) { window.suprimirProximoEventoRealtime = true; await db.from('partidas').update({ jogos_json: window.jogosDaRodada, artilheiros_json: artilheiros }).eq('id', window.partidaAtualId); }
...
function atualizarListaJogosDaRodada() {
    ...
    let validMatches = window.jogosDaRodada.filter(j => j.tipo !== 'modo');
    let qtdReg = document.getElementById('qtd-jogos-reg'); if(qtdReg) qtdReg.innerText = validMatches.length;
    
    if(validMatches.length === 0) { lista.innerHTML = '<p style="color:var(--text-muted); font-size:13px; text-align:center;">Nenhum jogo confirmado</p>'; return; }
    
    let htmlCompleto = ''; let numPartidaCount = 1;
    window.jogosDaRodada.forEach((j, originalIndex) => {
        ...
        if (j.tipo === 'ajuste') {
            let sinal = j.gols > 0 ? '+' : '';
            let cardHtml = `<div style="background: var(--white); padding: 12px; border-radius: 8px; border: 1px dashed var(--border); margin-bottom: 8px; display:flex; justify-content:space-between; align-items:center;"><div><span style="font-size:10px; color:var(--text-muted); font-weight:bold; display:block; text-transform:uppercase;">Ajuste Manual</span><strong>${escapeHTML(j.jogador)}</strong></div><div style="font-size:15px; font-weight:bold; color:${j.gols > 0 ? 'var(--supabase)' : 'var(--danger)'};">${sinal}${j.gols} gols</div>${btnExcluirHtml}</div>`;
            htmlCompleto = cardHtml + htmlCompleto;
        } else {
            let autoresA = formatarGolsResumo(j.gols_a); let autoresB = formatarGolsResumo(j.gols_b); let detalhesGolsHtml = '';
...
function renderizarPainelDoDiaComJogos(jogosArr, dataStr) {
    ...
    if(bodyClass) {
        ...
        else { let htmlClass = ''; rankTimes.forEach((r, i) => { let t = r[1]; let sg = t.gp - t.gc; let ic = i===0?'🏆 ':''; htmlClass += `<tr><td style="font-weight:700;">${ic}${escapeHTML(r[0])}</td><td style="font-weight:700; color:var(--primary);">${t.pts}</td><td>${t.j}</td><td>${t.v}</td><td>${t.e}</td><td>${t.d}</td><td>${sg > 0 ? '+'+sg : sg}</td></tr>`; }); bodyClass.innerHTML = htmlClass; }
    }
    ...
    if(bodyArts) {
        ...
        else { let htmlArts = ''; arts.forEach((a, i) => { let ic = i===0?'🥇':(i===1?'🥈':(i===2?'🥉':'⚽')); htmlArts += `<tr><td>${ic} <strong>${escapeHTML(a[0])}</strong></td><td>${a[1]}</td></tr>`; }); bodyArts.innerHTML = htmlArts; }
    }
}
...
async function carregarEstatisticasGerais() {
    ...
    try {
        let hoje = new Date(); let mesAtual = hoje.getMonth(); let anoAtual = hoje.getFullYear(); const startOfYear = new Date(anoAtual, 0, 1).toISOString();
        ...
        let mensalistasPorPartida = {}; let artilhariaMes = {}; let rankingAnual = {};
        if(presencas) { presencas.forEach(p => { if (p.jogadores && p.jogadores.user_id === currentUser.id && p.jogadores.tipo === 'Mensalista') { if(!mensalistasPorPartida[p.partida_id]) mensalistasPorPartida[p.partida_id] = new Set(); mensalistasPorPartida[p.partida_id].add(p.jogadores.nome); } }); }
        if(presencas && partidas) { presencas.forEach(p => { if (p.jogadores && p.jogadores.user_id === currentUser.id && p.jogadores.tipo === 'Mensalista') { let nome = p.jogadores.nome; if(!rankingAnual[nome]) rankingAnual[nome] = { gols: 0, presencas: 0 }; if(partidas.find(x => x.id === p.partida_id)) rankingAnual[nome].presencas++; } }); }
        if(partidas) { partidas.forEach(p => { let isEsteMes = new Date(p.data_sorteio).getMonth() === mesAtual; let arts = safeParse(p.artilheiros_json) || {}; let mensalistasNesta = mensalistasPorPartida[p.id] || new Set(); for(let nome in arts) { if(!(mensalistasNesta.has(nome) || jogadores.some(j => j.nome === nome && j.tipo === 'Mensalista'))) continue; let gols = arts[nome]; if(!rankingAnual[nome]) rankingAnual[nome] = { gols: 0, presencas: 0 }; rankingAnual[nome].gols += gols; if(isEsteMes) artilhariaMes[nome] = (artilhariaMes[nome] || 0) + gols; } }); }

        const bodyArtsMes = document.getElementById('body-artilharia-mes'); 
        if(bodyArtsMes) { let artsMesArr = Object.entries(artilhariaMes).sort((a,b) => b[1] - a[1]); if(artsMesArr.length === 0) bodyArtsMes.innerHTML = '<tr><td colspan="2" style="color:var(--text-muted);">Sem histórico neste mês.</td></tr>'; else { let htmlArtsMes = ''; artsMesArr.forEach((a, i) => { let ic = i===0?'🔥':'⚽'; htmlArtsMes += `<tr><td>${ic} <strong>${escapeHTML(a[0])}</strong></td><td>${a[1]}</td></tr>`; }); bodyArtsMes.innerHTML = htmlArtsMes; } }
